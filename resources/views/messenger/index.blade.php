...
@push('scripts')
    <script>
        ...
        // Helper function to decode names with 'fr' priority
        function getTranslatedName(name) {
            ...
            try {
                ...
                if (typeof decodedName === 'object' && decodedName !== null) {
                    // Prioritize 'fr', then fall back to the first available value or original
                    return decodedName.fr || Object.values(decodedName)[0] || name;
                }
            } catch (e) {
                console.warn(`⚠️ Failed to parse name as JSON: ${name}, error: ${e.message}`);
                return name; // Fall back to original name if parsing fails
            }
        ...
        async function loadConversations() {
            try {
                ...
                if (conversationsArray.length > 0) {
                    ...
                    updateUnreadBadges();
                    window.lastCheckTime = latestTimestamp;
                }
                displayConversations(conversations);
        ...
        async function loadFriendsData() {
            try {
                ...
                let friends = (data.friends || []).map(friend => {
                    ...
                    // Decode the friend’s name
                    friend.name = getTranslatedName(friend.name);
