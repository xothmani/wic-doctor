    ...
    <script>
        ...
        async function loadConversations() {
            try {
                ...
                // Helper function to decode names
                function getTranslatedName(name) {
                    if (!name) return 'Unknown User';

                    try {
                        const decodedName = JSON.parse(name);
                        const currentLocale = document.documentElement.lang || 'fr'; // Get current language from HTML lang attribute

                        if (typeof decodedName === 'object' && decodedName !== null) {
                            // Try current language first
                            if (decodedName[currentLocale]) {
                                return decodedName[currentLocale];
                            }
                            // Fall back to French
                            if (decodedName.fr) {
                                return decodedName.fr;
                            }
                            // Fall back to English
                            if (decodedName.en) {
                                return decodedName.en;
                            }
                            // Use first available value
                            const values = Object.values(decodedName);
                            if (values.length > 0) {
                                return values[0];
                            }
                        }
                    } catch (e) {
                        // If JSON parsing fails, return original name
                        return name;
                    }
                ...
                latestMessages.forEach((latestMessage, otherUserId) => {
                    try {
                        ...
                        // Decode the name using our helper function
                        const translatedName = getTranslatedName(otherUserInfo.name);

                        const conversationObj = {
                            id: latestMessage.conversation_id,
                            firebase_path: `/messages/${latestMessage.conversation_id}/chats/`,
                            name: translatedName, // Use translated name instead of raw name
                            avatar: otherUserInfo.imageUrl || otherUserInfo.avatar || '/images/default-avatar.png',
                            other_user: {
                                id: otherUserInfo.id,
                                name: translatedName, // Use translated name here too
