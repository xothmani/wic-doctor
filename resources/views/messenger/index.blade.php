                            ...
                            <div class="user-info">
                                ...
                                <div class="user-details">
                                    @php
                                        ...
                                        $name = (is_object($decodedName) && isset($decodedName->fr))
                                            ? $decodedName->fr
                                            : $userName;
                                    @endphp
...
@push('scripts')
    <script>
        ...
        // 🔧 Utility function to decode JSON names and combine first/last names
        function decodeName(name, lastname = '') {
            // Function to decode a single name field
            function decodeNameField(nameField) {
                if (!nameField) {
                    return '';
                }

                try {
                    // If the name is already a plain string without JSON, return it
                    if (typeof nameField === 'string' && !nameField.includes('{') && !nameField.includes('"')) {
                        return nameField;
                    }

                    // Handle multiple JSON objects in the same string
                    if (nameField.includes('} {')) {
                        const jsonStrings = nameField.split('} {').map((part, index, array) => {
                        ...
                        jsonStrings.forEach(jsonStr => {
                            try {
                                const decoded = JSON.parse(jsonStr.trim());
                                if (typeof decoded === 'object' && decoded !== null && decoded.fr) {
                                    decodedParts.push(decoded.fr);
                                }
                            } catch (e) {
                        ...
                        if (decodedParts.length > 0) {
                            return decodedParts[0]; // Take the first decoded part
                        }
                    }

                    // Try to parse as a single JSON object
                    const decoded = JSON.parse(nameField);
                    ...
                    // If JSON parsing fails, return the original name
                    return nameField;
                }
            }

            // Decode both name fields
            const decodedName = decodeNameField(name);
            const decodedLastname = decodeNameField(lastname);

            // Combine name and lastname with proper spacing
            const fullName = [decodedName, decodedLastname].filter(n => n && n.trim()).join(' ');
            ...
            try {
                ...
                latestMessages.forEach((latestMessage, otherUserId) => {
                    try {
                        ...
                        const decodedOtherUserName = decodeName(otherUserInfo.name) || `User ${otherUserInfo.id}`;
                        const conversationObj = {
                            id: latestMessage.conversation_id,
                            firebase_path: `/messages/${latestMessage.conversation_id}/chats/`,
                            name: decodedOtherUserName,
                ...
                if (conv.other_user && conv.other_user.id) {
                    ...
                    if (correctedAvatar === (conv.avatar || '/images/default-avatar.png') && window.friends && Array.isArray(window.friends)) {
                        ...
                        if (friend) {
                            ...
                            // Properly decode friend name with lastname if available
                            if (friend.lastname && friend.lastname !== null && friend.lastname !== 'null') {
                                let firstName = friend.name;
                                if (typeof friend.name === 'string' && friend.name.includes(' ')) {
                                    firstName = friend.name.split(' ')[0];
                                }
                                correctedName = decodeName(firstName, friend.lastname);
                            } else {
                                correctedName = decodeName(friend.name);
                            }
                        }
                    }
                }
        ...
        function displayFriends(friends) {
            ...
            friends.forEach(friend => {
                // Check if friend has separate firstname and lastname fields, or combined name
                let decodedName;
                if (friend.lastname && friend.lastname !== null && friend.lastname !== 'null') {
                    // If lastname exists separately, decode both and combine
                    let firstName = friend.name;
                    if (typeof friend.name === 'string' && friend.name.includes(' ')) {
                        firstName = friend.name.split(' ')[0]; // Take first part as first name
                    }
                    decodedName = decodeName(firstName, friend.lastname);
                } else {
                    // If no separate lastname, decode the full name field
                    decodedName = decodeName(friend.name);
                }
                ...
                html += `
                                <div class="friend-item ${pendingClass}" onclick="startChatWithFriend(${friend.id}, '${decodedName.replace(/'/g, "\\'")}', '${friend.avatar}', ${isPending})">
                                    <img src="${friend.avatar || '/images/default-avatar.png'}" class="contact-avatar" alt="${decodedName}">
                                    <div class="contact-info">
                                        <h6 class="contact-name">${decodedName} ${statusBadge}</h6>
        ...
        function openConversation(conversationId, name, avatar) {
            ...
            // Decode the name in case it contains JSON formatting
            const decodedName = decodeName(name);
            document.getElementById('chat-contact-name').textContent = decodedName;
        ...
        async function acceptFriendRequest(friendId) {
            try {
                ...
                if (response.ok) {
                    ...
                    const friendName = decodeName(responseData.data?.friend_name) || 'Docteur';
                    ...
                    if (window.friends && Array.isArray(window.friends)) {
                        ...
                        if (friend) {
                            ...
                            friendElements.forEach(element => {
                                ...
                                // Decode the friend name properly with lastname if available
                                let decodedName;
                                if (friend.lastname && friend.lastname !== null && friend.lastname !== 'null') {
                                    let firstName = friend.name;
                                    if (typeof friend.name === 'string' && friend.name.includes(' ')) {
                                ...
                                if (nameElement) {
                                    nameElement.innerHTML = decodedName;
                                }
                                ...
                                element.setAttribute('onclick', `startChatWithFriend(${friend.id}, '${decodedName.replace(/'/g, "\\'")}', '${friend.avatar}', false)`);
                            });
                        }
                    }
                } else {
        ...
        function displayFirebaseMessage(messageData, isOwn = false, isGroupConversation = false) {
            ...
            const senderName = decodeName(messageData.sender?.name) || 'Unknown';
