...
/////////////////////////////////////////////
// New channel for audit logs
Broadcast::channel('audit-logs.{doctorId}', function ($user, $doctorId) {
    \Log::info('Checking audit-log channel access', [
        'user_id' => $user->id,
        'doctor_id' => $doctorId,
        'user_roles' => $user->roles->pluck('name'),
    ]);

    // Allow the doctor to listen to their own audit logs
    if ($user->hasRole('doctor')) {
        $doctor = $user->doctor; // Get the doctor record associated with the user  
    ...
    // Allow the secretary to listen to audit logs for their associated doctor
    if ($user->hasRole('Secretary')) {
        $associatedDoctor = $user->associatedDoctors()->where('doctor_id', $doctorId)->first();
    ...
    // Allow telesecretaries to listen to audit logs for their selected doctor
    if ($user->hasRole('Telesecretary')) {
        $isSelectedDoctor = session('selectedDoctorId') == $doctorId;
